...
def open_positions(client):
    ...
    # Get markets fr referencing of min order size, tick size etc
        ...
        if len(series_1) > 0 and len(series_1) == len(series_2):
            ...
            if abs(z_score) > ZSCORE_THRESH:
                ...
                if not is_base_open and not is_quote_open:
                    ...
                    # Get acceptable price in string format with correct nuber of decimals
                    ...
                    accept_base_failsafe_price = format_number(failsafe_base_price, base_tick_size)
                    ...
                    quote__min_order_size = markets['markets'][quote_market]['minOrderSize']
                    check_base = float(base_quantity) > float(base_min_order_size)
                    check_quote = float(quote_quantity) > float(quote__min_order_size)
                    ...
                    bot_agent = BotAgent(
                        ...
                        accept_failsafe_base_price=accept_base_failsafe_price,
                        z_score=z_score,
                        half_life=half_life,
